		...
		<div class="row">
							...
							<div style="margin-bottom: 10px;">
								<small>
									@foreach($game->publishers->shuffle() as $key => $publisher)
										...
										@else
											<span class="hidden-xs"><a href="/publishers/{{ $publisher->slug }}" itemprop="publisher" itemscope itemtype="http://schema.org/Organization"><span itemprop="name">{{ $publisher->name }}</span></a>,</span>
										@endif
									@endforeach
          ...
          <div class="row">
            <div class="btn-group btn-group-justified col-xs-12" role="group">
				...
				@if(!empty($game->link))
					<a href="{!! $game->link !!}" target="_blank" class="btn btn-ocean text-uppercase hidden-xs" style="font-size:13px;" title="Search on Board Game Shopper"><i class="fa fa-shopping-cart" aria-hidden="true"></i> <span class="hidden-sm hidden-xs">Find </span>Online</a>
				@endif
				@if(Session::has('name'))
