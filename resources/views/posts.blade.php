						...
						<div class="row post">
							...
							@else
								<div class="col-sm-3 col-xs-12 hidden-xs" style="padding: 15px;">
									<a href="/{{ $category->slug }}/{{ $post->slug }}" title="{{ $post->games()->orderBy(DB::raw('RAND()'))->first()->name }}">
										<img src="https://assets.ozboardgamer.com{{ $post->games()->orderBy(DB::raw('RAND()'))->first()->thumb1x }}" srcset="https://assets.ozboardgamer.com{{ $post->games()->orderBy(DB::raw('RAND()'))->first()->thumb1x }} 1x, https://assets.ozboardgamer.com{{ $post->games()->orderBy(DB::raw('RAND()'))->first()->thumb2x }} 2x" alt="{{ $post->games()->orderBy(DB::raw('RAND()'))->first()->name }}" class="img-responsive" itemprop="image" style="margin: auto;" />
									</a>								
								</div>
								<div class="col-sm-9 col-xs-12">
							@endif
